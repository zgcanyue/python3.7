...
#修改blog API
@post('/api/blogs/{id}')
async def api_updata_blog(id,request,*,name,summary,content):
    check_admin(request) #检查是否为管理员
    blog = await Blog.find(id) #数据库中查找blog id,等待返回
    if not name or not name.strip():
        raise APIValueError('name','name cannot be empty.')
    if not summary or not summary.strip():
        raise APIValueError('summmar','summary cannot be empty.')
...
async def api_delate_blog(request,*,id):
    check_admin(request)
    blog = await Blog.find(id) #找到指定blog的id
    await blog.remove() #删除该条blog
    return dict(id=id)
